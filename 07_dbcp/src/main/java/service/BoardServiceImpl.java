...
public class BoardServiceImpl implements BoardService {
  ...
  public ActionForward getBoardList(HttpServletRequest request) {
    ...
    Optional<String> opt = Optional.ofNullable(request.getParameter("page")); // 페이지 번호를 받아옴
    int page = Integer.parseInt(opt.orElse("1"));                             // 전달된 페이지 번호가 없는 경우 1을 전달
    
    int total = dao.getBoardCount();  // dao 필요함 (전체 게시글 수는 DB에서 가져옴)
    ...
    // DB로부터 게시글 목록 가져오기 (Dao 필요)
    List<BoardDto> boardList = dao.getBoardList(map);
    
    // 게시글 목록을 /board/list.jsp로 전달하기 위하여 request에 저장한 뒤 forward한다.
